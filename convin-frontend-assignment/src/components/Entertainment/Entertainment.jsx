...
class Entertainment extends Component {
    ...
    fetchEntertainmentAPI(){
        fetch("http://localhost:8000/entertainmentData")
        .then(res => res.json())
        .then(res=>this.setState({entertainmentCardData:res}));
    }

    componentWillMount(){
        this.fetchEntertainmentAPI();
    }

    renderEntertainmentData()
    {
        return this.state.entertainmentCardData.map(data => (
    ...
    handleSubmit = async e => {
        e.preventDefault();
        const response = await fetch('http://localhost:8000/entertainmentData', {
          method: 'POST',
          headers: {
    ...
    deleteCard(e){
        fetch(`http://localhost:8000/entertainmentData/${e}`,{
            method: 'DELETE',
        }).then((res) =>{
            res.json().then((repo) =>{
            })
    ...
    handleUpdate(id, cardName, mp3Link) {
        ...
        fetch(`http://localhost:8000/entertainmentData/${id}`,{
            method: 'PUT',
            headers: {
    ...
    moveToSportsCard(id, cardName, mp3Link)
    {
        ...
        //delete card from entertainment bucket
        fetch(`http://localhost:8000/entertainmentData/${id}`, {
            method: 'DELETE',
        }).then((res) => {
            res.json().then((repo) => {
    ...
    render() {
        return (
            <div>
                ...
                <div class="row entertainment-container" style={{margin:'3rem'}}>
                    <div class="card col-2" style={{marginRight: '1rem', marginTop: '1rem'}}>
                        <div class="card-body">
                        <Popup trigger=
                                { <button type="submit" class="btn">
